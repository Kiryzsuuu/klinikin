  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/patient-auth/register", {
        ...
        body: JSON.stringify({ ...form, clinicSlug: slug }),
      });
      ...
      router.push(`/c/${slug}/portal/verify-otp?medicalRecordNo=${encodeURIComponent(form.medicalRecordNo)}`);
    } finally {
      setLoading(false);
    }
  }
      ...
      <Card className="w-full max-w-md">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            ...
            <Input id="mrn" required placeholder="KK-2026-000001" value={form.medicalRecordNo} onChange={(e) => setForm({ ...form, medicalRecordNo: e.target.value })} />
          </div>
          <div>
        ...
        <p className="text-sm text-dark/60 mt-6 text-center">
          Sudah punya akun? <Link href={`/c/${slug}/portal/login`} className="text-green font-medium">Masuk</Link>
        </p>
        <p className="text-xs text-dark/40 mt-2 text-center">
          Belum pernah berobat? <Link href={`/c/${slug}/booking`} className="text-green">Booking konsultasi dulu</Link>
        </p>
      </Card>
    </main>
  );
