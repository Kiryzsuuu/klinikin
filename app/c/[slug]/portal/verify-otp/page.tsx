...
function VerifyForm({ slug }: { slug: string }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/patient-auth/verify-otp", {
        ...
        body: JSON.stringify({ medicalRecordNo, code, purpose, clinicSlug: slug }),
      });
      ...
      if (purpose === "PATIENT_RESET_PASSWORD") {
        router.push(`/c/${slug}/portal/reset-password?medicalRecordNo=${encodeURIComponent(medicalRecordNo)}&code=${code}`);
      } else {
        router.push(`/c/${slug}/portal/dashboard`);
      }
    } finally {
      setLoading(false);
    }
...
export default function PortalVerifyOtpPage({ params }: { params: Promise<{ slug: string }> }) {
